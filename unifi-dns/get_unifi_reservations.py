...
def create_hosts(hosts, domain: str, reverse: bool=False):
  ...
  for ip, host, comment in new_hosts:
    ...
    if reverse:
        hosts.reverse()
    new_host_text += f"{ip} {' '.join(hosts)} {comment if comment else ''}\n"
  # End for
...
def main():
  ...
  parser.add_argument('-r', '--reverse', action='store_true', default=False, help='Reverse the order of the hosts so the FQDN comes first')
  ...
  create_hosts(hosts, args.domain, args.reverse)
# End def
